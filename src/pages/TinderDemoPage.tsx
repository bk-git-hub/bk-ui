...
// 코드 보기에 표시될 예제 코드 문자열
const usageCode = `
import { Tinder, useTinderContext } from "@/components/Tinder";

const YourComponent = () => {
  const cards = [/* ... 카드 데이터 배열 ... */];

  // 카드가 없을 때 보여줄 UI (선택 사항)
  const EmptyState = () => (
    <div className="flex h-full w-full items-center justify-center">
      <h2 className="text-xl font-bold">모든 카드를 확인했습니다!</h2>
    </div>
  );
  ...
  const TinderContent = () => {
    ...
    return isFinished ? <EmptyState /> : <CardStack />;
  }

  return (
    <div className="flex flex-col items-center">
      <Tinder.Root cards={cards} className="h-[550px] w-96">
        <TinderContent />
      </Tinder.Root>

      <div className="mt-8 flex space-x-8">
        <Tinder.NopeButton>Nope</Tinder.NopeButton>
        <Tinder.LikeButton>Like</Tinder.LikeButton>
      </div>
    </div>
  );
};
