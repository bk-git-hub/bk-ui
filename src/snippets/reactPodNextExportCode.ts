...
export const reactPodNextJsExport: ComponentViewerCodeTab = {
  ...
  description:
    "App Router can render the interactive ReactPod through its client entry while page.tsx remains a Server Component. Pass serializable data across that boundary and add a consumer 'use client' wrapper only when supplying function-valued props.",
...
// - Keep menuItems, photoAlbums, and initial props deterministic between the
//   server render and hydration. Shuffle randomness runs only after selection.
// - Server Components may pass serializable menu and photo data. Put native
//   event handlers or other function-valued props in your own "use client"
//   wrapper rather than passing functions across the server/client boundary.

// Tailwind CSS v4
// Local files below app or src are detected automatically. For an external
// package, add both sources relative to app/globals.css (adjust for src/app).
//
// app/globals.css
// @import "tailwindcss";
