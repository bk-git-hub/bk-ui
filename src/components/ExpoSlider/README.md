...
| `velocityThreshold`      | `0.45`       | 속도 투영을 적용할 최소 px/ms       |
...
전환 중에도 이전/다음 버튼, 방향키, 페이지네이션 입력을 다시 받을 수 있습니다. 새 입력은 현재 시각 진행률에서 하나의 최종 목표로 애니메이션을 재지정하므로 중간 인덱스의 Expo 효과를 건너뛰지 않고, 완료 콜백도 최종 값에 대해 한 번만 발생합니다. 애니메이션 중 새 드래그를 시작하면 진행 축이 확정되는 순간 현재 시각 위치를 이어 잡고 기존 목표에 새 드래그 의도를 누적합니다.

포인터 드래그는 진행 축을 판별하기 위한 7px 방향 잠금 이후 별도의 슬라이드 이동 임계값 없이 커밋됩니다. 긴 드래그는 거리를 기준으로 여러 인덱스를 한 번에 이동하며, `velocityThreshold` 이상의 같은 방향 속도는 최대 한 인덱스만 추가로 투영합니다. 비루프 모드에서는 첫/마지막 인덱스까지만 이동하고 끝의 시각적 저항도 제한됩니다.

이전 버전과의 타입 호환을 위해 `dragThreshold` prop은 deprecated 상태로 남아 있지만 더 이상 동작에 영향을 주지 않습니다.
