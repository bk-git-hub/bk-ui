...
`spinning` prop으로 챔버의 혼합 상태를 표현할 수 있고, `renderBall`, `getItemKey`, `getItemLabel`, `ballClassName`으로 임의 타입의 공을 렌더링합니다. 혼합 중에는 각 공에 중력과 지속적인 상승류·회전 난류가 적용되며, 혼합을 멈추면 공이 바닥으로 자연스럽게 가라앉습니다. 기본적으로 공을 5개의 깊이 레이어로 나누고 같은 레이어의 공끼리만 충돌시켜 입체적인 겹침과 가벼운 움직임을 만듭니다. `depthLayers`로 1~6개 사이의 레이어 수를 조정하고 `motionSeed`로 같은 초기 배치와 움직임을 재현할 수 있습니다. 이 물리 효과는 추첨 난수를 소비하지 않으며, 사용자가 모션 감소를 요청한 환경에서는 정적인 깊이 배치로 표시합니다.
